...
for OG_ID, group in grouped_df:
  ...
  for pair in filt_gene_pairs:
    ...
    if geneID1 not in list(joint_expr_table.index.values) or geneID2 not in list(joint_expr_table.index.values):
      ...
      expr_sim = compute_pairwise_expr_sim(geneID1_expr, geneID2_expr)
      #Join to final_df
    final_df = pd.concat([final_df, pd.DataFrame({"OG_ID" : [OG_ID, OG_ID], "Species1" : [species1, species2], "Species2" : [species2, species1], "GeneID1" : [geneID1, geneID2], "GeneID2" : [geneID2, geneID1], "Expr_sim" : [expr_sim, expr_sim]})])
