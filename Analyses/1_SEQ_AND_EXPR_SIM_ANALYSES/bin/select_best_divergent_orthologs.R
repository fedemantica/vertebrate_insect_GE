...
##### Define arguments
#Create a parser
p = arg_parser("Select Best-Divergent paralogs within vertebrates and insects. Importantly, the outpupt orthology will not include genes for the outgroups")
...
p = add_argument(p, arg="--output", help="Output file with best-divergent orthogroups")
p = add_argument(p, arg="--output_complete_BD", help="Output file with best-divergent orthogroups and seq sim values")
...
output_complete_BD_file = argv$output_complete_BD
...
best_divergent_complete_df = average_seq_sim_df %>%
  group_by(OG_ID, Species, Clade) %>%
  filter(Average_seq_sim == min(Average_seq_sim)) %>%
  ungroup() %>%
...
best_divergent_df = best_divergent_complete_df %>%
  dplyr::select(OG_ID, Species, GeneID)

#######################
##### Save output #####
#######################
write.table(best_divergent_df, output_file, col.names=FALSE, row.names=FALSE, quote=FALSE, sep="\t")
write.table(best_divergent_complete_df, output_complete_BD_file, col.names=FALSE, row.names=FALSE, quote=FALSE, sep="\t")
