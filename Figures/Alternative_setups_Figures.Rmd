...
##### Create run-specific variables
paralog_type = "ALL"
sequence_similarities_type = "sequence_similarities_original_NoStartEndGaps"
expression_similarities_type = "expression_similarities"
...
input_seq_sim_df = read.delim(paste0(general_patterns_dir, paralog_type, "/", sequence_similarities_type, "/", sequence_similarities_type, "_by_clade-", paralog_type, "_genes.tab"), header=TRUE)
colnames(input_seq_sim_df) = c("OG_ID", "sequence_similarities_vertebrata", "sequence_similarities_insecta", "sequence_similarities_delta")
...
input_expr_sim_df = read.delim(paste0(general_patterns_dir, paralog_type, "/", expression_similarities_type, "/", expression_similarities_type, "_by_clade-", paralog_type, "_genes.tab"), header=TRUE)
colnames(input_expr_sim_df) = c("OG_ID", "expression_similarities_vertebrata", "expression_similarities_insecta", "expression_similarities_delta")
...
features = c(sequence_similarities_type, expression_similarities_type)
    ...
    for (feature in features) {
      ...
      input_df = read.delim(input_file, header=FALSE, col.names=c("OG_ID")) %>%
        mutate(Conservation=status,
               Clade=clade,
               Feature=sub("_original.*", "", sub("_blosum.*", "", sub("_summed_paralogs", "", feature))))
...
## SUMMED_PARALOG_EXPR ###
##########################
# I need a slightly different code here, because the expression quantification is done at the level of orthogroups.
if (expression_similarities_type == "expression_similarities_summed_paralogs") {
  
  #### Read in taus
  taus_df = data.frame()
  for (species in c(ordered_species[ordered_species!="BmA"], "Bmo")) {
    taus_file = paste0("/users/mirimia/fmantica/projects/vertebrate_insect_GE/data/paralog_sets/pairwise_expression_similarities_summed_paralogs/species_taus/", species, "-taus.tab")
    species_taus_df = read.delim(taus_file, header=FALSE, col.names=c("OG_ID", "Tau")) %>%
      mutate(Species = species)
    ##### Join to final df
    taus_df = dplyr::bind_rows(species_taus_df, taus_df)
  }
...
pdf(paste0("/users/mirimia/fmantica/projects/vertebrate_insect_GE/tmp_images/all_extra_whole_figures/", paralog_type, "-", sequence_similarities_type, "-", expression_similarities_type, "-NES_legend-v1.pdf"), 
    width=10, height=10)
print(NES_plots)
dev.off()
...
### This is different for the summed paralogs
if (expression_similarities_type == "expression_similarities_summed_paralogs") {
  all_ts_genes_df = data.frame()
  for (species in c(ordered_species[ordered_species != "BmA"], "Bmo")) {
    tau_file = paste0("/users/mirimia/fmantica/projects/vertebrate_insect_GE/data/paralog_sets/pairwise_expression_similarities_summed_paralogs/species_taus/", species, "-taus.tab")
    ts_tissue_file = paste0("/users/mirimia/fmantica/projects/vertebrate_insect_GE/data/paralog_sets/pairwise_expression_similarities_summed_paralogs/species_associated_tissue/", species, "-associated_tissue.tab")
    #### Read in files
    tau_df = read.delim(tau_file, header=FALSE, col.names=c("OG_ID", "Tau"))
    associated_tissue_df = read.delim(ts_tissue_file, header=FALSE, col.names=c("OG_ID", "Tissue", "Top_tissue"))
    #### Combine inputs and process
    combined_df = tau_df %>%
      filter(Tau >= 0.75) %>%
      left_join(associated_tissue_df, by="OG_ID") %>%
      filter(Tissue != "None") %>%
      separate_rows(Tissue, sep=";") %>%
      mutate(Species = species) %>%
      mutate(Clade = ifelse(Species %in% Vertebrata, "Vertebrata", "Insecta"))
    #### Join to final dataframe
    all_ts_genes_df = all_ts_genes_df %>%
      dplyr::bind_rows(combined_df)
  }
}
...
pdf(paste0("/users/mirimia/fmantica/projects/vertebrate_insect_GE/tmp_images/all_extra_whole_figures/", 
           paralog_type, "-", sequence_similarities_type, "-", expression_similarities_type, "-whole_figure-v1.pdf"), width=17/2.54, height=26/2.54)
print(paralog_set_figure)
dev.off()
```
